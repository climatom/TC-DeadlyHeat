...
def write_NC(oname,met_vars,latin,lonin,idin,timein,time_units,stdvin):
    ...
    stdv = ncfile.createVariable('stdv',np.float32,dimensions=['id',])
...
# Repeat for orog stdv
stdfile=Dataset(stdv_file,"r")
stdv=np.squeeze(stdfile.variables["sdor"][:,:,:])
stdv_out=stdv[locs[:,7].astype(np.int),locs[:,8].astype(np.int)]
...
fail=write_NC(oname,\
              {"hi":[out_hi,"degrees_Celsius"],\
               ...
               "days since 1979-1-1 00:00:00",stdv_out)
